...
class SLM(object):
    ...
    def __init__(self, numb,NT,N,initial_phi=None, profile_s=None):
        
        self.n_pixelsx = int(N[0]) 
        self.n_pixelsy = int(N[1]) 
        self.intensity_calc = None
        
        self.cost = None # placeholder for cost function.
        
        if profile_s is None:
            profile_s = torch.ones((self.n_pixelsx, self.n_pixelsy)) # input amplitude set to flat ones if none given
        if initial_phi is None:
            initial_phi = torch.rand((self.n_pixelsx*self.n_pixelsy), dtype=torch.float32) * 2 * torch.pi # input phase set to random if none given
        if not torch.is_complex(profile_s):
            profile_s = profile_s.to(torch.complex128)
        assert profile_s.shape == (self.n_pixelsx, self.n_pixelsy), 'profile_s is wrong shape, should be ({n},{n})'.format(n=self.n_pixelsx)
        self.profile_s_r = profile_s.real.type(torch.float32)
        self.profile_s_i = profile_s.imag.type(torch.float32)
        
        assert initial_phi.shape == (self.n_pixelsx*self.n_pixelsy,), "initial_phi must be a vector of phases of size N^2 (not (N,N)).  Shape is " + str(initial_phi.shape)

        # Linked to the fourier transform. Keeps the same quantity of light between the input and the output
        self.A0 = 1./np.sqrt(NT[0]*NT[1])
        
        # Set zeros matrix:
        self.zero_frame = torch.zeros((NT[0], NT[1]), dtype=torch.float32)
        self.zero_matrix = torch.as_tensor(self.zero_frame, dtype=torch.float32)
        
        # Phi and its momentum for use in gradient descent with momentum:
        self.phi = torch.tensor(initial_phi, dtype=torch.float32, requires_grad=True)
        self.phi_rate = torch.zeros_like(self.phi, dtype=torch.float32)
        self.phi_reshaped = self.phi.view(self.n_pixelsx, self.n_pixelsy)
        
        # E_in (n_pixels**2):
        self.S_r = torch.tensor(self.profile_s_r, dtype=torch.float32)
        self.S_i = torch.tensor(self.profile_s_i, dtype=torch.float32)
        self.E_in_r = self.A0 * (self.S_r * torch.cos(self.phi_reshaped) - self.S_i * torch.sin(self.phi_reshaped))
        self.E_in_i = self.A0 * (self.S_i * torch.cos(self.phi_reshaped) + self.S_r * torch.sin(self.phi_reshaped))
        
        # E_in padded (4n_pixels**2):
        idx_0x, idx_1x = get_centre_range(self.n_pixelsx,NT[0])
        idx_0y, idx_1y = get_centre_range(self.n_pixelsy,NT[1])
        self.E_in_r_pad = self.zero_matrix.clone()
        self.E_in_r_pad[idx_0x:idx_1x, idx_0y:idx_1y] = self.E_in_r
        self.E_in_i_pad = self.zero_matrix.clone()
        self.E_in_i_pad[idx_0x:idx_1x, idx_0y:idx_1y] = self.E_in_i
        self.phi_padded = self.zero_matrix.clone()
        self.phi_padded[idx_0x:idx_1x, idx_0y:idx_1y] = self.phi_reshaped

        ################################################################
        # E_out:
        self.E_out_r, self.E_out_i = (fft(self.E_in_r_pad, self.E_in_i_pad))
        # Output intensity:
        self.E_out_2 = self.E_out_r ** 2 + self.E_out_i ** 2
        
        # E_out_phi:
        self.E_out_p = torch.atan2(self.E_out_i, self.E_out_r)
        self.E_out_p_nopad = self.E_out_p[idx_0x:idx_1x, idx_0y:idx_1y]
        
        # Output amplitude:
        self.E_out_amp = torch.sqrt(self.E_out_2)

#########################    end SLM class    ##########################
########################################################################
    def update_output(self,NT=[2514,2514]):
        self.phi_rate = torch.zeros_like(self.phi, dtype=torch.float32)
        self.phi_reshaped = self.phi.view(self.n_pixelsx, self.n_pixelsy)
...
def gaussian_line(dim, r0, d, sigma, A=1.0, save_param=False, device='cuda'):
    """
    Create n x n target: 
    ...
    z = A * torch.exp(-(fx**2 + (Y - r0[1])**2) / sigma**2)
...
def gaussian_line2(dim, r0, d, sigmax,sigmay, A=1.0, save_param=False, device='cuda'):
    """
    Create n x n target: 
    Gaussian line centered on r0 = (x0, y0) with length 'd', width
    'sigma', and amplitude 'A'.
    ...
    # Target definition
    fx = 0.5 * (torch.abs(X - d / 2. - r0[0]) + torch.abs(X + d / 2. - r0[0]) - d)
    z = A * torch.exp(-(fx**2/ sigmax**2 + (Y - r0[1])**2/ sigmay**2) )

    if save_param:
        param_used = f"gaussian_line | n={rows} | r0={r0} | d={d} | sigma={sigmax} | A={A}"
        return z, param_used
    else:
        return z

def gaussian_top_square(dim, r0, dx, dy, sigmax, sigmay, A=1.0, save_param=False):
    """
    Create n x n target: 
    ...
    device = torch.device("cuda")
    torch.set_default_device(device)
    cols, rows = dim  # 从输入中获取矩阵的行和列
    x = torch.arange(rows, dtype=torch.float32, device=device)  #
    y = torch.arange(cols, dtype=torch.float32, device=device)  #
    X, Y = torch.meshgrid(x, y, indexing='xy')  # 2D grid
    z = torch.zeros((cols,rows))

    # target definition
    fx = 0.5*(torch.abs(X-dx/2.-r0[0])+torch.abs(X+dx/2.-r0[0])-dx)
    fy = 0.5*(torch.abs(Y-dy/2.-r0[1])+torch.abs(Y+dy/2.-r0[1])-dy)
    z = A*torch.exp(-( torch.pow(fx/sigmax,2.)+torch.pow(fy/sigmay,2.)))
    z[z<1E-5]=0

    if save_param :
        param_used = "gaussian_line | n={0} | r0={1} | dx={2} | dy={3} | sigmax={4} | sigmay={5}| A={6}".format(rows, r0, dx, dy, sigmax, sigmay, A)
        return z, param_used
    else :
        return z


def gaussian_top_round(dim, r0, d, sigma, A=1.0, save_param=False):
    """
    Create n x n target:
    ...
    Args:
        n (int): Size of the grid (n x n).
        r0 (tuple): Center of the circle (x0, y0).
        d (float): Diameter of the circle.
        sigma (float): Tail width of the Gaussian wings.
        A (float): Amplitude of the Gaussian.
        save_param (bool): Whether to return parameters used.

    Returns:
        torch.Tensor: 2D Gaussian target.
        str (optional): Parameters used (if save_param is True).
    """
    # Initialization
    device = torch.device("cuda")
    ...
    y = torch.arange(cols, dtype=torch.float32, device=device)  #
    X, Y = torch.meshgrid(x, y, indexing='xy')  # 2D grid
    r0 = torch.tensor(r0, dtype=torch.float32)  # Center as tensor
    z = torch.zeros((rows, cols), dtype=torch.float32)  # Initialize the result grid
    
    # Compute distance from the center
    r = torch.sqrt((X - r0[0]) ** 2 + (Y - r0[1]) ** 2)

    # Target definition
    inter = 0.5 * (torch.abs(r - d / 2) + torch.abs(r + d / 2) - d)
    z = A * torch.exp(-torch.pow(inter / sigma, 2))
    
    # Set small values to zero
    z[z < 1E-5] = 0

    if save_param:
        param_used = f"gaussian_top_round | n={rows} | r0={r0.tolist()} | d={d} | sigma={sigma} | A={A}"
        return z, param_used
    else:
        return z


def flat_top_round(dim, r0, d, A=1.0, save_param=False):
    """
    Create n x n target: 
    ...
    if save_param :
        param_used = "flat_top_round | n={0} | r0={1} | d={2} | A={3}".format(rows, r0, d, A)
        return z, param_used
    else :
        return z
...
def phase_flat(dim, v, save_param=False, device='cpu'):
    """
    Create n x n target phase:
    ...
    Args:
        n (int): Size of the grid (n x n).
        v (float): Value of the flat phase.
        save_param (bool): Whether to return parameters used.
        device (str): Device to store tensors, e.g., 'cpu' or 'cuda'.

    Returns:
        torch.Tensor: 2D flat phase.
        str (optional): Parameters used (if save_param is True).
    """
    # Initialize a flat tensor filled with 1.0
    cols, rows = dim
    device = torch.device("cuda")
    torch.set_default_device(device)
    z = torch.ones((cols, rows), dtype=torch.float32, device=device) * v

    if save_param:
        param_used = f"phase_flat | n={rows} | v={v}"
        return z, param_used
    else:
        return z
...
def phase_inverse_square(dim, r0, save_param=False):
    """
    Create n x n target using PyTorch:
    ...
    y = torch.arange(cols, dtype=torch.float32, device=device)  #
    X, Y = torch.meshgrid(x, y, indexing='xy')  
    r = torch.sqrt((X - r0[0]) ** 2 + (Y - r0[1]) ** 2)  # 计算距离 r
    
    # Target definition
    inverse_square = torch.abs((r + 1) ** -0.5)  # 计算反平方
    inverse_square = inverse_square / torch.max(inverse_square)  # 归一化
    z = inverse_square * (torch.pi / 2)  # 计算相位
    
    if save_param:
        param_used = f"phase_inverse_square_torch | n={cols} | r0={r0}"
        return z, param_used
    else:
        return z
...
def weighting_value(M, p, v=0, save_param=False):
    """
    Apply weighting to a matrix M with a parameter p and threshold v.

    Args:
        M (torch.Tensor): Input tensor (matrix).
        p (float): Proportionality constant to calculate the threshold.
        v (float, optional): Value to assign for elements below the threshold. Default is 0.
        save_param (bool, optional): Whether to return the parameters used. Default is False.
    ...
    device = torch.device("cuda")
    torch.set_default_device(device)
    M = torch.as_tensor(M, dtype=torch.float32)  # Ensure M is a tensor
    z = torch.ones_like(M)  # Initialize with ones

    # Calculate threshold value and apply the weighting
    v1 = p * M.max()
    z[torch.abs(M) < v1] = v

    if save_param:
        param_used = f"weighting_value | p={p} | v={v}"
        return z, param_used
    else:
        return z
...
def give_plot_scale(M, p, extension):
    ...
    return int(imin +i_trans), int(imax+i_trans), int(jmin+j_trans), int(jmax+j_trans)
...
def Fidelity(weighting, target, phase, E_out_amp, E_out_p):
    # 计算加权的目标和输出场
    E_test_w = (target * torch.exp(1j * phase)) * weighting
    E_out_w = (E_out_amp * torch.exp(1j * E_out_p)) * weighting

    # 计算内积
    numerator = torch.sum(E_test_w * torch.conj(E_out_w))

    # 计算分母的模平方和平方根
    denominator = torch.sqrt(
        torch.sum(torch.abs(E_test_w) ** 2) * torch.sum(torch.abs(E_out_w) ** 2)
    )
...
def RMS_error(weighting, I_target, I_out):
    ...
    # 非零权重数量
    MR = torch.count_nonzero(weighting)

    # 归一化
    I_target_w_norm = I_target_w / torch.sum(I_target_w)
    I_out_w_norm = I_out_w / torch.sum(I_out_w)

    # 计算误差
    n = torch.pow((I_out_w_norm - I_target_w_norm) / I_target_w_norm, 2)

    # 处理 NaN 和 Inf
    n = torch.nan_to_num(n, nan=0.0, posinf=0.0, neginf=0.0)

    # 计算 RMS 误差
    n = torch.sqrt(torch.sum(n) / MR)
...
def Phase_error(weighting, phase, E_out_p):
    ...
    # 计算 P2
    P2 = torch.where(
        torch.abs(phase_w - E_out_p_w) >= torch.pi,
        (2 * (E_out_p_w <= phase).float() - 1) * 2 * torch.pi * weighting,
        torch.zeros_like(phase_w)
    )

    # 计算相位误差
    e = torch.sum(torch.pow(torch.abs(E_out_p_w - phase_w + P2), 2)) / torch.sum(torch.pow(torch.abs(phase_w), 2))
...
def Efficiency(weighting, I_out):
    ...
    I_out_w_tot = torch.sum(I_out*weighting)
