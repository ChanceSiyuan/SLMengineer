...
def SLM_screen_Correct(SLM_screen,LUT=224,correctionImgPath='calibration/CAL_LSH0804730_850nm.bmp'):
    # 该函数对WGS算法生成的相位图做了修正，考虑了出厂自带的correction和LUT校正
    try:
        correction=np.array(Image.open(correctionImgPath))
    except:
        print("Correction image not found.")
        correction=np.zeros_like(SLM_screen)
    SLM_screen = SLM_screen.astype(np.uint8)
    correction = correction.astype(np.uint8)
    SLM_screen_Corrected=SLM_screen+correction
    SLM_screen_Corrected_LUT=(SLM_screen_Corrected/256*LUT).astype('uint8')
