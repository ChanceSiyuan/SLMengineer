...
def WGS_phase_generate(initSLMAmp, initSLMPhase, targetAmp, Loop=5, threshold=0.01, Plot=False):
    ...
    targetAmp : 2D_tensor
        The amplitude distrubution of the target. May be real-valued or
        complex-valued; if complex, only ``|targetAmp|`` is used and the
        phase is ignored (WGS does not use target phase).
    ...
    # WGS only uses target amplitude; collapse complex input to its magnitude.
    if torch.is_complex(targetAmp):
        targetAmp = torch.abs(targetAmp)
...
def WGS3D_phase_generate(initSLMAmp, initSLMPhase, targetAmp, targetLayer, Loop=5, threshold=0.01, Plot=False):
    ...
    targetAmp : 3D_tensor
        ...
        May be real or complex; if complex, only ``|targetAmp|`` is used.
