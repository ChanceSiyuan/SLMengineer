...
This script runs ENTIRELY on the local (Linux+RTX 3090) box.  It does:
  ...
  8. Saves three artefacts in ``scripts/`` (so they survive the
     ``ai_slm_loop.sh`` tar exclusions where applicable):
       ...
       scripts/testfile_lg_payload.npz      (slm_screen uint8 + diagnostics)
       scripts/testfile_lg_params.json      (human-readable metadata)
       scripts/testfile_lg_preview.pdf      (6-panel visualisation)
...
Next step after running this script::
    ...
    ./scripts/testfile_lg.sh   # pushes payload + triggers remote runner
...
PAYLOAD_PATH = "scripts/testfile_lg_payload.npz"
PARAMS_PATH = "scripts/testfile_lg_params.json"
PREVIEW_PATH = "scripts/testfile_lg_preview.pdf"
