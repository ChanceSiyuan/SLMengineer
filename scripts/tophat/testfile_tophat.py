"""Local-only CGM compute: produce a top-hat phase payload for the
dedicated Windows hardware runner.

Parallel to ``scripts/testfile_lg.py`` but targeting a circular flat-top
via ``SLM.top_hat_target``.  Everything else (compute grid, CGM config,
Fresnel + calibration pipeline, save format) is held constant so the
per-shape diff on the camera is attributable to the target choice alone.

Next step after running this script::

    ./push_run.sh payload/tophat/testfile_tophat_payload.npz
"""
...
OUTPUT_DIR = "payload/tophat"
PAYLOAD_PATH = f"{OUTPUT_DIR}/testfile_tophat_payload.npz"
PARAMS_PATH = f"{OUTPUT_DIR}/testfile_tophat_params.json"
PREVIEW_PATH = f"{OUTPUT_DIR}/testfile_tophat_preview.pdf"
...
def main():
    ...
    # --- Top-hat target parameters (1024^2 grid, focal pitch 15.83 um/px) ---
    # radius=10 focal-px = ~158 um on focal plane = ~86 camera-px diameter on the 1.85 um camera.
    tophat_radius = 10.0   # pixels on 1024 grid (~158 um in focal plane)
    ...
    cgm_D = -np.pi / 12     # smaller linear phase offset (~950 um shift, on-camera)
    ...
    # --- 2. Generate top-hat target on the 1024x1024 computation grid ---
    ...
    # --- 3. Build the paper's analytical initial phase (on 1024^2 grid) ---
    ...
    # --- 7. Diagnostic metrics (final fidelity, efficiency on the 1024 grid) ---
