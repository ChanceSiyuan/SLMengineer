#!/bin/bash
# testfile_tophat.sh — local CGM -> push to Windows -> run hardware -> pull results
#
# Parallel to testfile_lg.sh for the top_hat_target shape.  Pipeline:
#   [1/4] Local:  uv run python scripts/tophat/testfile_tophat.py
#                 -> produces scripts/tophat/testfile_tophat_payload.npz
#                             scripts/tophat/testfile_tophat_params.json
#                             scripts/tophat/testfile_tophat_preview.pdf
...
PREFIX="testfile_tophat"
PAYLOAD="scripts/tophat/${PREFIX}_payload.npz"
PARAMS="scripts/tophat/${PREFIX}_params.json"

# ─── Step 1: Local CGM compute ───────────────────────────────────────
echo "[1/4] Running CGM locally (~100 s on 4096^2 RTX 3090)..."
uv run python scripts/tophat/testfile_tophat.py

if [ ! -f "${PAYLOAD}" ]; then
    echo "ERROR: ${PAYLOAD} not produced by testfile_tophat.py" >&2
    exit 1
fi
